...
class znMessageQueue{
	...
	public:
		...
		inline bool							send(void *msg, uint32_t wait_time = SYNCTIMEOUT_INFINITY) {return zunoSysCall(ZUNO_SYSFUNC_MSGQUEUE_SEND, 3, _handle, msg, wait_time) != (void*)0;};
		inline bool							receive(void *msg, uint32_t wait_time = SYNCTIMEOUT_INFINITY) {return zunoSysCall(ZUNO_SYSFUNC_MSGQUEUE_RCV, 3, _handle, msg, wait_time) != (void*)0;};
	private:
		void *_handle;
		_znSysQData_t  _queue_data;
};
...
class znThread {
	public:
		...
		inline void							suspend() {zunoSysCall(ZUNO_SYSFUNC_THREAD_SUSPEND, 1, _handle);};
		inline void							resume() {zunoSysCall(ZUNO_SYSFUNC_THREAD_RESUME, 1, _handle);};
		inline bool							isValid() {return _handle != NULL;};
...
inline void zunoSuspendThread(void *handle) {
	zunoSysCall(ZUNO_SYSFUNC_THREAD_SUSPEND, 1, handle);
}
