...
int zuno_CCManufacturerSpecificHandler(const ZUNOCommandCmd_t *cmd, 
    ...
    frame_report->info.packet.zw_rx_secure_opts = rx_s2level;

	switch(ZW_CMD){
        case MANUFACTURER_SPECIFIC_GET:
            frame_report->info.packet.cmd[2] = (uint8_t)(DEVICE_MANUFACTURER_ID >> 8);
            frame_report->info.packet.cmd[3] = (uint8_t)(DEVICE_MANUFACTURER_ID &  0xFF);
            frame_report->info.packet.cmd[4] = (uint8_t)(DEVICE_PRODUCT_TYPE_ID  >> 8);
            frame_report->info.packet.cmd[5] = (uint8_t)(DEVICE_PRODUCT_TYPE_ID  &  0xFF);
        ...
        case DEVICE_SPECIFIC_GET:
            {
                ...
                frame_report->info.packet.cmd[2] = (uint8_t)(DEVICE_ID_TYPE_SERIAL_NUMBER & 0x07);
                frame_report->info.packet.cmd[3] = (uint8_t)(((DEVICE_ID_FORMAT_BINARY << 0x05) & 0xE0) | DEVICE_UID_LEN);
