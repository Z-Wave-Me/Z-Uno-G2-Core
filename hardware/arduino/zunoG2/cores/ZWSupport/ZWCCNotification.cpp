...
int zuno_CCNotificationReport(byte channel, ZUNOCommandPacket_t *cmd){
	...
	zunoEEPROMRead(NOTIFICATION_CC_EEPROM, 4, (byte*)&eeprom_mask);// Load report mask from EEPROM
	...
	memset(report, 0, sizeof(report->byte4));// Initially till the report data with zeros
	index = (ZUNO_CFG_CHANNEL(channel).sub_type) << 1;
	if(cmd != NULL) {
		cmd_get = (ZwNotificationGetFrame_t*)cmd->cmd;
		if((NOTIFICATION_MAPPER[index] != cmd_get->notificationType) && (cmd_get->notificationType != 0xFF))
...
static int _supported_get(size_t channel) {
	...
	report->cmdClass = COMMAND_CLASS_NOTIFICATION;
	report->cmd = NOTIFICATION_SUPPORTED_REPORT;
	report->properties1 = NOTIFICATION_SUPPORTED_DATA_LEN;
	
	CMD_REPLY_LEN = NOTIFICATION_SUPPORTED_LEN;
	CMD_REPLY_DATA(0) = NOTIFICATION_SUPPORTED_DATA_LEN;
	zunoSetupBitMask(&(CMD_REPLY_DATA(1)), NOTIFICATION_MAPPER[(ZUNO_CFG_CHANNEL(channel).sub_type) << 1], NOTIFICATION_REPORT_DATA_LEN);
	return (ZUNO_COMMAND_ANSWERED);
}
