...
ZUNO_CUSTOM_CC(
	...
	ZUNO_CC_VERSION(COMMAND_CLASS_CENTRAL_SCENE, CENTRAL_SCENE_VERSION)
);

ZUNO_SETUP_CENTRAL_SCENE((CENTRAL_SCENE_KEY_PRESSED_1_MASK | CENTRAL_SCENE_KEY_RELEASED_MASK | CENTRAL_SCENE_KEY_HELD_DOWN_MASK | CENTRAL_SCENE_KEY_PRESSED_2_MASK | CENTRAL_SCENE_KEY_PRESSED_3_MASK),
...
ZUNO_SETUP_ENTRY_CONTROL_EVENT(ENTRY_CONTROL_NOTIFICATION_EVENT_RFID_MASK);
ZUNO_SETUP_ENTRY_CONTROL_DATA(ENTRY_CONTROL_NOTIFICATION_DATA_RAW_MASK);
...
void process_buttons() {
	if (long_click == true && Btn.isReleased(BUTTON)) {
		zuno_CCCentralSceneReport(0x1, CENTRAL_SCENE_KEY_RELEASED);
		long_click = false;
		MY_SERIAL.println("isReleased");
	}
	if(Btn.isSingleClick(BUTTON)) {
		zuno_CCCentralSceneReport(0x1, CENTRAL_SCENE_KEY_PRESSED_1);
		MY_SERIAL.println("isSingleClick");

	}
	if(Btn.isTripleClick(BUTTON)) {
		zuno_CCCentralSceneReport(0x1, CENTRAL_SCENE_KEY_PRESSED_3);
		MY_SERIAL.println("isTripleClick");

	}
	if(Btn.isLongClick(BUTTON)) {
		zuno_CCCentralSceneReport(0x1, CENTRAL_SCENE_KEY_HELD_DOWN);
		long_click = true;
		MY_SERIAL.println("isLongClick");

	}
	if(Btn.isDoubleClick(BUTTON)) {
		zuno_CCCentralSceneReport(0x1, CENTRAL_SCENE_KEY_PRESSED_2);
