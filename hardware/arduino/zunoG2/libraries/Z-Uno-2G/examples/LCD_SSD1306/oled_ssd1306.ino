...
void setup()
{
  ...
  lcd.clearDisp();
	...
	lcd.write("Привет Мир Проверка МНОГО! много текста\nYou hohoO");
  // lcd.setFont((uint8_t*)fontMK_907x7);
	// lcd.write("Hop");
  // for (int i= 0; i < 10;i++)
  // {
  //   lcd.writePixel(i,i,1);
  // }
  // lcd.writePixel(0,0,1);
	// lcd.clearDisp();
...
//   for(int16_t i=0; i<max(lcd.width(),lcd.height())/2 - 2; i+=2) {
//     lcd.drawCircle(lcd.width()/2 - 1, lcd.height()/2 - 1, i, SSD1306_WHITE);
//     lcd.display();
//     delay(1);
//   }
...
//     lcd.fillCircle(lcd.width() / 2 - 1, lcd.height() / 2 - 1, i, SSD1306_INVERSE);
