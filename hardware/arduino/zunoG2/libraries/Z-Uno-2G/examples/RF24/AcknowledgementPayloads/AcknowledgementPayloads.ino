...
void setup() {
  ...
  MY_SERIAL.println(F("RF24/examples/AcknowledgementPayloads"));
...
void loop() {
  ...
  if (role) {
    ...
    if (report) {
      ...
      MY_SERIAL.print(payload.message);                         // print the outgoing message
      MY_SERIAL.print(payload.counter);                         // print the outgoing counter
      uint8_t pipe;
      if (radio.available(&pipe)) {                          // is there an ACK payload? grab the pipe number that received it
        PayloadStruct received;
        radio.read(&received, sizeof(received));             // get incoming ACK payload
        MY_SERIAL.print(F(" Recieved "));
        MY_SERIAL.print(radio.getDynamicPayloadSize());         // print incoming payload size
        MY_SERIAL.print(F(" bytes on pipe "));
        MY_SERIAL.print(pipe);                                  // print pipe number that received the ACK
        MY_SERIAL.print(F(": "));
        MY_SERIAL.print(received.message);                      // print incoming message
        MY_SERIAL.println(received.counter);                    // print incoming counter

        // save incoming counter & increment for next outgoing
        payload.counter = received.counter + 1;

      } else {
        MY_SERIAL.println(F(" Recieved: an empty ACK packet")); // empty ACK packet received
      }
