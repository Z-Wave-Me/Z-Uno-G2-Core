...
void setup() {
  ...
  MY_SERIAL.println(F("RF24/examples/MulticeiverDemo"));
  MY_SERIAL.println(F("*** Enter a number between 0 and 5 (inclusive) to change"));
  MY_SERIAL.println(F("    the identifying node number that transmits."));
...
void loop() {
  ...
  if (role <= 53) {
    ...
    if (report) {
      ...
      MY_SERIAL.print(F("Transmission of payloadID "));
      MY_SERIAL.print(payload.payloadID);                       // print payloadID
      MY_SERIAL.print(F(" as node "));
      MY_SERIAL.print(payload.nodeID);                          // print nodeID
      MY_SERIAL.print(F(" successful!"));
      MY_SERIAL.print(F(" Time to transmit: "));
      MY_SERIAL.print(end_timer - start_timer);                 // print the timer result
      MY_SERIAL.println(F(" us"));
    } else {
    ...
    if (radio.available(&pipe)) {             // is there a payload? get the pipe number that recieved it
      ...
      MY_SERIAL.print(F(" from node "));
      MY_SERIAL.print(payload.nodeID);           // print the payload's origin
      MY_SERIAL.print(F(". PayloadID: "));
      MY_SERIAL.println(payload.payloadID);      // print the payload's number
    }
  } // role
  ...
  if (MY_SERIAL.available()) {
    ...
    if (toupper(c) == 'R' && role <= 53) {
      ...
      role = 'R';
      MY_SERIAL.println(F("*** CHANGING ROLE TO RECEIVER ***"));
      MY_SERIAL.println(F("--- Enter a number between 0 and 5 (inclusive) to act as"));
      MY_SERIAL.println(F("    a unique node number that transmits to the RX node."));
      ...
      role = c - 48;
      MY_SERIAL.print(F("*** CHANGING ROLE TO NODE "));
      MY_SERIAL.print(c);
      MY_SERIAL.println(F(" ***"));
      MY_SERIAL.println(F("--- Enter a number between 0 and 5 (inclusive) to change"));
      MY_SERIAL.println(F("    the identifying node number that transmits."));
      MY_SERIAL.println(F("--- PRESS 'R' to act as the RX node."));
...
void setRole() {
  if (role == 'R') {
    ...
    radio.openWritingPipe((uint8_t)address[(uint8_t)role]);
